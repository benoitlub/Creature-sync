...
function ScannerLines() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" style={{ zIndex: 1 }}>
      <div className="absolute inset-0" style={{ backgroundImage: "repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0,0,0,0.03) 2px, rgba(0,0,0,0.03) 4px)", backgroundSize: "100% 4px" }} />
    </div>
  );
}
...
function FeuchEmblem() {
  return (
    <div className="hidden xs:flex w-12 h-12 shrink-0 items-center justify-center rounded-xl border" style={{ borderColor: "#00d4ff55", background: "radial-gradient(circle at 50% 50%, rgba(0,212,255,0.12), rgba(2,8,20,0.3))", boxShadow: "0 0 18px #00d4ff22, inset 0 0 16px #00d4ff11", clipPath: "polygon(18% 0, 82% 0, 100% 30%, 100% 70%, 82% 100%, 18% 100%, 0 70%, 0 30%)" }}>
...
function LangSelector({ lang, onChange }: { lang: Lang; onChange: (l: Lang) => void }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      {opts.map(o => <button key={o.v} onClick={() => onChange(o.v)} className="text-[9px] font-mono px-1.5 py-0.5 rounded border transition-all duration-200" style={{ borderColor: lang === o.v ? "#00d4ff55" : "#ffffff11", color: lang === o.v ? "#00d4ff" : "#ffffff44", background: lang === o.v ? "rgba(0,212,255,0.08)" : "transparent", boxShadow: lang === o.v ? "0 0 6px #00d4ff22" : "none" }}>{o.l}</button>)}
    </div>
  );
}
...
function Header({ glitch, lang, onLangChange }: { glitch: boolean; lang: Lang; onLangChange: (l: Lang) => void }) {
  ...
  return (
    <header className="relative px-3 pt-3 pb-2" style={{ zIndex: 3 }}>
      <div className="relative overflow-hidden rounded-b-xl border px-3 py-3" style={{ borderColor: "#00d4ff33", background: "linear-gradient(180deg, rgba(2,10,24,0.98), rgba(0,12,30,0.86))", boxShadow: "0 0 22px rgba(0,212,255,0.12), inset 0 0 22px rgba(0,212,255,0.05)" }}>
        ...
        <div className="flex items-center gap-3"><FeuchEmblem /><div className="min-w-0 flex-1"><div className="text-xl sm:text-2xl font-mono font-bold tracking-[0.22em] uppercase leading-none truncate" style={{ color: "#00d4ff", textShadow: "0 0 14px #00d4ff99", filter: glitch ? "blur(0.5px)" : "none" }}>{t.title}</div><div className="text-[8px] sm:text-[9px] font-mono tracking-[0.34em] text-orange-400/70 uppercase mt-1 truncate">{t.subtitle}</div></div></div>
        <div className="mt-3 flex items-center justify-between gap-2 text-[9px] font-mono text-gray-500 tracking-wider flex-wrap"><LangSelector lang={lang} onChange={onLangChange} /><span className="hidden sm:inline text-gray-700">|</span><span>{t.institute}</span><div className="flex items-center gap-1 ml-auto"><div className="w-1.5 h-1.5 rounded-full" style={{ background: blink ? "#00ff88" : "#00ff8844", boxShadow: blink ? "0 0 6px #00ff88" : "none", transition: "all 0.3s" }} /><span style={{ color: blink ? "#00ff88" : "#00ff8844" }}>{t.online}</span></div></div>
      </div>
    </header>
  );
}
...
function getBiologicalActivityPercent(audioFeatures: any, progress: number) {
  if (!audioFeatures) return Math.min(42, Math.max(0, Math.round(progress * 0.35)));
  const tonal = Math.max(0, 1 - (audioFeatures.flatness ?? 1));
  const repetition = Math.min(1, (audioFeatures.periodicity ?? 0) * 3.4);
  const articulation = Math.min(1, (audioFeatures.zcr ?? 0) * 9.5);
  const highBand = Math.min(1, Math.max(0, ((audioFeatures.spectralCentroid ?? 0) - 650) / 2600));
  const antiRumble = 1 - Math.min(0.55, Math.max(0, ((audioFeatures.lowEnergyRatio ?? 0) - 0.58) * 1.3));
  const whisperPresence = (audioFeatures.rms ?? 0) > 0.0012 ? 0.12 : 0.04;
  const biological = (tonal * 0.24 + repetition * 0.31 + articulation * 0.2 + highBand * 0.13 + whisperPresence) * antiRumble;
...
function SpeciesCandidateList({ candidates, active }: { candidates: LiveCandidate[]; active: boolean }) {
  ...
  return (
    <div className="rounded border px-3 py-2 backdrop-blur-sm" style={{ borderColor: active ? "#00ff8840" : "#ffffff11", background: "rgba(0,10,25,0.58)" }}>
      <div className="flex items-center justify-between mb-1.5">
        <div className="text-[9px] font-mono tracking-[0.32em] uppercase text-green-300/70">Espèces probables</div>
        <div className="text-[8px] font-mono tracking-[0.22em] uppercase text-white/30">BirdNET-lite</div>
      </div>
      <div className="space-y-1">
        {rows.length === 0 ? (
          <div className="grid grid-cols-[18px_1fr_42px] items-center gap-2 text-[9px] font-mono tracking-wider text-gray-600"><span>--</span><span className="truncate uppercase">Aucune espèce stabilisée</span><span className="text-right">--</span></div>
        ) : rows.map((candidate, index) => (
          <div key={`${candidate.id}-${index}`} className="grid grid-cols-[18px_1fr_auto_38px] items-center gap-2 text-[9px] font-mono tracking-wider">
            <span className="text-green-300/70">{index + 1}.</span>
            <span className="truncate uppercase text-gray-300">{candidate.name}</span>
            <LedBar value={candidate.confidence} />
            <span className="text-right text-green-200">{candidate.confidence}%</span>
          </div>
        ))}
      </div>
    </div>
...
function LiveSignalDashboard({ active, audioFeatures, detectedLabel, progress }: { active: boolean; audioFeatures: any; detectedLabel: string | null; progress: number }) {
  ...
  return (
    <div className="rounded border px-3 py-2 backdrop-blur-sm" style={{ borderColor: active ? "#00d4ff44" : "#ffffff11", background: "rgba(0,10,25,0.68)" }}>
      <div className="flex items-center justify-between mb-1.5"><div className="text-[9px] font-mono tracking-[0.32em] uppercase text-cyan-400/70">Signatures & activité biologique</div><div className="text-[8px] font-mono tracking-[0.22em] uppercase" style={{ color: active ? "#00ff88" : "#ffffff33" }}>{active ? "LIVE" : "VEILLE"}</div></div>
      <div className="space-y-1">{signatureRows.map(row => { const diode = row.tone === "bio" ? "#00ff88" : row.tone === "primary" ? "#00ff88" : row.tone === "secondary" ? "#00d4ff" : row.tone === "warning" ? "#ff8c00" : "#ffffff33"; return <div key={row.label} className="grid grid-cols-[10px_1fr_auto_34px] items-center gap-2 text-[9px] font-mono tracking-wider"><span className="w-1.5 h-1.5 rounded-full" style={{ background: diode, boxShadow: row.pending ? "none" : `0 0 5px ${diode}` }} /><span className={`truncate uppercase ${row.pending ? "text-gray-600" : "text-gray-300"}`}>{row.label}</span><LedBar value={row.value} pending={row.pending} /><span className={`text-right ${row.pending ? "text-gray-600" : "text-cyan-300"}`}>{row.pending ? "--" : `${row.value}%`}</span></div>; })}</div>
    </div>
  );
}
  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden flex flex-col" style={{ background: "radial-gradient(ellipse at 20% 20%, rgba(0,40,80,0.4) 0%, transparent 60%), radial-gradient(ellipse at 80% 80%, rgba(20,0,40,0.3) 0%, transparent 60%), #02060f", fontFamily: "'JetBrains Mono', 'Fira Code', 'Consolas', monospace" }}>
      ...
      <main className="relative flex-1 flex flex-col gap-2 p-3 sm:p-4 max-w-4xl mx-auto w-full" style={{ zIndex: 2, paddingBottom: "calc(6rem + env(safe-area-inset-bottom, 0px))" }}>
        <div className="grid grid-cols-2 gap-2"><SpeciesPanel state={state} lang={lang} /><EmotionalPanel state={state} lang={lang} /></div>
        ...
        <SpeciesCandidateList candidates={liveCandidates} active={state.isListening || state.isAnalyzing} />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2"><ThreatPanel state={state} lang={lang} /><BiologicalPanel state={state} lang={lang} /><NeuralPanel state={state} lang={lang} /><EnvironmentPanel state={state} lang={lang} /></div>
