...
type SignatureRow = {
  label: string;
  value: number;
  tone: "primary" | "secondary" | "warning" | "idle" | "bio";
  pending?: boolean;
};
  ...
  return (
    <div className="rounded border px-3 py-2 backdrop-blur-sm" style={{ borderColor: active ? "#00d4ff44" : "#ffffff11", background: "rgba(0,10,25,0.68)" }}>
      <div className="flex items-center justify-between mb-1.5">
        <div className="text-[9px] font-mono tracking-[0.32em] uppercase text-cyan-400/70">Signatures & activité biologique</div>
      ...
      <div className="space-y-1">
        {signatureRows.map(row => {
          const diode = row.tone === "bio" ? "#00ff88" : row.tone === "primary" ? "#00ff88" : row.tone === "secondary" ? "#00d4ff" : row.tone === "warning" ? "#ff8c00" : "#ffffff33";
          return (
